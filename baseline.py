...
DEFAULT_TASK = os.getenv("TASK_NAME", "hard")
...
def run_episode(task_name: str = DEFAULT_TASK, num_emails: int = NUM_EMAILS) -> dict[str, Any]:
    ...
    while True:
        ...
        print(
            f"  Step {step+1:02d} | "
            f"cls={'ok' if info.get('classification_correct') else 'x'} "
            f"risk={'ok' if info.get('risk_correct') else ('-' if risk is None else 'x')} "
            f"dec={'ok' if info.get('decision_correct') else ('-' if dec is None else 'x')} "
